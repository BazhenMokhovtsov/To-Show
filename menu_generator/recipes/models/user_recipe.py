...
def upload_recipe_image(instance, filename):
    return f"user_recipes/{instance.user.username}/{filename}"
...
class UserRecipe(models.Model):
    ...
    products = models.ManyToManyField(
        Product, related_name="user_recipes", blank=True, verbose_name="Продукты"
    )
    json_products = models.JSONField(
        default=dict, null=False, blank=True, verbose_name="JSON продуктов"
    )

    image = models.ImageField(
        upload_to=upload_recipe_image, null=True, blank=True, verbose_name="Изображение"
    )
    type = models.CharField(max_length=50, choices=TYPE_RECIPE.items(), verbose_name="Тип рецепта")
    meal_time = MultiSelectField(choices=MEAL_TIME.items(), verbose_name="Время приёма")
    instructions = models.ManyToManyField(
        Instruction, related_name="user_recipes", blank=True, verbose_name="Инструкция"
    )
    ...
    def save(self, *args, **kwargs):
        # Преобразуем обратно из строки в словарь
        if isinstance(self.json_products, str):
            self.json_products = json.loads(self.json_products)

        # Дальше можно работать с json_products как с dict
        self.json_products = {
...
def update_json_products(sender, instance, **kwargs):
    ...
    UserRecipe.objects.filter(pk=instance.pk).update(
...
@receiver(m2m_changed, sender=UserRecipe.products.through)
